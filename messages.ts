...
export abstract class Message {
    ...
    constructor(public dst: number = 0, public src: number = 0) {}
    ...
    toBinary(): Uint8Array {
        ...
        if (payload.length === 0) {
            ...
            throw new MessageTooLarge("Message's length is too large");
        }
    ...
    static fromBinary(data: Uint8Array): Message {
        ...
        // console.log(`Signature: ${signature} src: ${src}, dst: ${dst}, len: ${msgLength}, type: ${typecode}, data: ${payload}, dataLen: ${payload.byteLength}, crc: ${crc}`)
        const msgClass: Message = typeCode2Class[typecode](payload, dst, src);
        const crcCheck: Uint8Array = msgClass.zcrc16(
            new Uint8Array([dst, src, payload.length + 1, typecode, ...payload])
        );
...
export class TextMessage extends Message {
    constructor(public readonly message: string, src: number = 0, dst: number = 0) {
        super(src, dst);
    }
...
export const setReadMode = (): number => {
...
const FileErrorToString = (error: number): string => {
...
export class FileClose extends Message {
    constructor(public readonly handle: number, dst: number, src: number) {
        super(dst, src);
    }
...
export class FileCloseResponse extends Message {
    ...
    static fromPayload(payload: Uint8Array, dst: number, src: number): FileClose {
        const errorCode = payload[0];
        Message.getMessageError(errorCode);
        return new FileClose(errorCode, dst, src);
    }
}

class FileWrite extends Message {
    constructor(
        public readonly handle: number,
...
class FileWriteResponse extends Message {
    constructor(
        public readonly errorCode: number,
        public readonly handle: number,
        public readonly position: number,
...
> = {
    ...
    [MessageTypes.FILE_OPEN]: FileOpen.fromPayload,
    [MessageTypes.FILE_OPEN_RESPONSE]: FileOpenResponse.fromPayload,
    [MessageTypes.FILE_CLOSE]: FileClose.fromPayload,
    [MessageTypes.FILE_CLOSE_RESPONSE]: FileCloseResponse.fromPayload,
    [MessageTypes.FILE_WRITE]: FileWrite.fromPayload,
    [MessageTypes.FILE_WRITE_RESPONSE]: FileWriteResponse.fromPayload,
    [MessageTypes.FILE_READ]: FileRead.fromPayload,
    [MessageTypes.FILE_CONTENT]: FileContent.fromPayload,
};
...
class MessageTooLarge extends MessageError {
    //Message's length is too large
    constructor(message: string) {
        super(message);
...
class UnknownMessageType extends MessageError {
    //Unknown message typecode
    constructor(message: string) {
        super(message);
